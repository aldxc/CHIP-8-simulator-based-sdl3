...
bool Platform::initialize(int scale) {
    ...
    // 创建窗口
    window_ = SDL_CreateWindow("CHIP-8", windowWidth_, windowHeight_, SDL_WINDOW_SHOWN);
    if (!window_) {
        SDL_Quit();
        return false;
    }
...
int Platform::mapKey(SDL_Keycode key) const {
    // 常见键位映射：
    // 键盘布局:
...
void Platform::render(const std::array<bool, kDisplayWidth * kDisplayHeight>& display, bool displayDirty) {
    ...
    SDL_Rect dstRect = { 0, 0, windowWidth_, windowHeight_ };
